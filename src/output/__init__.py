...
def create_output_tariff_navi(
    config_path, gd, target_month, branch_code
) -> DataFrame:
    """
    ...
    cost_df = cost_master.select(
        ...
        F.explode(
            F.array(
                F.lit("3商品"), F.lit("ドライ"), F.lit("冷蔵"), F.lit("冷凍")
            )
        ),
    )
    ...
    dm = target_cost_df.join(
        dm, ["顧客コード", "サイズ", "着地域名", "商品"], "left_outer"
    )
    ...
    VALUE_COLS_TITLE = [
        "単価（円）",
        "個あたり精緻化粗利（円）：発B以降の精緻化原価で計算",
        "個あたりハキダシ粗利（円）",
        "合計精緻化粗利（円）：発B以降の精緻化原価で計算",
        "合計ハキダシ粗利（円）",
        "個数（個）",
        "収入合計（円）",
    ]
    ...
    def create_formatted_excel(pdf: pd.DataFrame) -> pd.DataFrame:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            for product in PRODUCT_LIST:
                ...
                if not product_df.empty:
                    ...
                    total_hakidashi_profit = product_df[
                        "合計ハキダシ粗利"
                    ].sum()
                    seichi_profit_rate = (
                        float(total_seichi_profit) / total_revenue
                        if total_revenue != 0
                        else 0
                    )
                    hakidashi_profit_rate = (
                        float(total_hakidashi_profit) / total_revenue
                        if total_revenue != 0
                        else 0
                    )
                else:  # データがない場合は全て0
                ...
                ws["A1"] = "タリフナビ"
                ws["C1"] = (
                    "法人顧客の収入・個数・単価・粗利を見える化し、単価交渉の際のヒントとしてご活用ください。"
                )
                ws["M1"] = "社外秘"
                ...
                ws["C5"] = (
                    "(※1)台帳登録店、ただし製販分離適用顧客コードの場合は営業店"
                )
                ws["J3"] = "(※2)精緻化原価とは"
                ws["J4"] = (
                    "経営情報の機能別原価をベースにし、不在原価なども考慮し再計算した原価のこと"
                )
                ws["J5"] = (
                    "ハキダシよりも実態に近い、より精緻な原価を目指して算出したもの"
                )
                ...
                for value_col, title in zip(VALUE_COLS, VALUE_COLS_TITLE):
                    ...
                    if value_col == "単価":
                        ...
                        if not product_df.empty:
                            ...
                            pivot = pd.DataFrame(
                                0, index=SIZE_ORDER, columns=REGION_ORDER
                            )
                ...
                ws["M1"].font = Font(
                    size=18, name="Arial", bold=True, color="FF0000"
                )
            ...
            if gemini_flag and gemini_results is not None:
                ...
                # セクションタイトル
                ws.cell(
                    row=current_row, column=2, value="Gemini分析結果"
                ).font = title_font
                current_row += 1

                # gemini結果を書き込み
                gemini_cell = ws.cell(
                    row=current_row, column=2, value=gemini_results
                )
        ...
        return pd.DataFrame(
            ...
            columns=["file_path", "file_content"],
        )

    print(" 顧客ごとにExcelファイル生成処理を分散実行します...")

    # 顧客コードでグループ化し、各グループに対してPandas UDFを適用
    excel_results_df = dm.groupBy("顧客コード", "店所コード").applyInPandas(
        create_formatted_excel, schema=result_schema
    )
    ...
    try:
        ...
        print(
            f"主管フォルダ '{branch_code}' の作成中にエラーが発生しました: {e}"
        )
    ...
    excel_upload_df = (
        excel_results_df.withColumn(
            "upload_customer_folder",
            get_customer_folder_name(F.col("file_path")),
        )
        .withColumn(
        ...
        .withColumn(
            "branch_code",
            F.lit(branch_code),  # スクリプト冒頭のパラメータを列として追加
        )
    )
